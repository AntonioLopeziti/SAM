...
$(document).ready(function () {
    $('#iconmsg').hide();
    $('#PrintEtiqueta').click(function () {
        var datos = "&Orden=" + $('#Orden').val()
                  + "&Cliente=" + $('#Cliente').val()
                  ...
                  + "&Ancho=" + $('#Ancho').val() 
                  + "&Unidad=" + $('#Unidad').val() 
                  + "&PtoTrabajo=" + $('#PtoTrabajo').val() 
                  + "&Ruta=" + $('#Ruta').val() 
                  + "&Stock=" + $('#Stock').val() 
        var acc = "ImpimirEtiqueta";
        $.ajax({
            async: false,
            type: 'GET',
            url: 'pruebasServlet',
            ...
            success: function (data) {
                ...
                if(data == 1){
                    alert("No hay Servicio impresora o no esta conectada, verificar....");
                }
                if(data == 2){
                    alert("Mandado a imprimir");
                }
                if(data == 3){
                    alert("Error en el servicio de impresora");
                }
            }
        });
    });
