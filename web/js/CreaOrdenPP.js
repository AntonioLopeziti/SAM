...
function invalidEquip() {
    var msj = 'Equipo Invalido';
    $('#iconmsg').show();
    $('#iconmsg').attr('src', 'images/advertencia.PNG');
    $('#msg').html(msj);
}
...
//FUNCIONES Cargar Datos del input de Material
function cargarMat() {
    acc = "cargarDataMaterial";
    var equi = $('#Equipo').val();
    $.ajax({
        async: true,
        type: 'GET',
...
function delComponentes(noOpe) {
    var rows = rowsOfTable("TablaComponentes");
    var nCompOpe = document.getElementsByName('TCope');
    for (var i = 0; i < rows; i++) {
        try {
            if (nCompOpe[i].value == noOpe) {
...
function actualizarServicios() {
    ...
    var contO = $("input[name^='TOcont']");
    var contS = $("input[name^='TScont']");
    var opeS = $("input[name^='TSnoOpe']");
    var opeO = $("input[name^='TOope']");
    for (var i = 0; i < rowsO; i++) {
...
function actualizarMateriales() {
    ...
    var contO = $("input[name^='TOcont']");
    var contM = $("input[name^='TMcont']");
    var opeM = $("input[name^='TMnoOpe']");
    var opeO = $("input[name^='TOope']");
    for (var i = 0; i < rowsO; i++) {
...
function invalidCellObli(type, pos) {
    var obj = $('#' + type + pos);
    obj.focus();
    var menCam = "Faltan Datos Obligatorios";
    $('#iconmsg').show();
    $('#iconmsg').attr('src', 'images/advertencia.PNG');
    $('#msg').html(menCam);
}
...
function limpiarTabOpe() {
    $("#TablaStatusOpe").load("CrearOrden.jsp #TablaOperaciones");
    setTimeout(function () {
...
function validarNumEquiHR() {
    ...
    if (equi.val().length < 1) {
        ...
        var check = getExisNumMat(equi.val());
        if (check == 1) {
            ConsultarContaHR();
        } else if (check == 0) {
            invalidEquip();
...
function checkRowDbOpe(pos) {
    ...
    var acc = "checkRowOpe";
    var dataSend = "&rowOPto=" + ptoT + "&rowOCent=" + cent + "&rowOCls=" + cls + "&rowOUmd=" + umd.val();
    var rowP;
    $.ajax({
        async: false,
        type: 'GET',
        ...
        $.ajax({
            ...
            success: function (rs) {
                if (rs == "x") {
                    ...
                    $('#TCumc' + posMat).prop('disabled', true);
...
function insertarOrdenAllCheck() {
    var check1 = checkEmptyTableComp();
...
function insertarOrdenAll() {
    var check1 = checkEmptyTableComp();
    var check2 = checkEmptyTableServi();
    var fol = "OR" + obtenerFolio();
    var folNum = obtenerFolio();
    guardarOpe(fol);
    insertarDatCab(fol);
    insertarTxtsD(fol);
    if (check1 == 1) {
        var checkPM = checkPMope();
        if (checkPM == 0) {
        } else {
            guardarComp(fol);
        }
...
function updateFolio(fol) {
    ...
    $.ajax({
        ...
        contentType: "application/x-www-form-urlencoded",
        processData: true,
        data: "acc=" + acc + "&folAct=" + fol,
        success: function (rs) {
            //            alert(rs);
        }
    });
}
function cleanTotal(fol) {
    limpiarPrn();
    limpiarSer();
    limpiarTabComp();
    limpiarTabOpe();
    ...
    rowMate = 0;
    rowComp = 0;
    var menCam = "Orden Creada - " + fol;
    $('#iconmsg').show();
    $('#iconmsg').attr('src', 'images/aceptar.png');
    $('#msg').html(menCam);

}
function limpiarTabTxtD() {
    $("#TablaStDescri").load("CrearOrden.jsp #TablaStDescri");
}
...
function insertarDatCab(fol) {
    ...
    var datosSend = "&cabClo=" + claseOr + "&cabDes=" + encodeURIComponent(description) + "&cabEqui=" + equi + "&cabUbiT=" + ubiTec + "&cabCentP=" + centP + "&cabPtoR=" + ptoRes + "&cabHor=" + hora + "&cabFech=" + fecha + "&cabFechIni=" + ini + "&cabFechFin=" + fin + "&folio=" + fol + "&cabUs=" + usuario;
    $.ajax({
        async: false,
        type: 'GET',
        url: 'peticionModuloCrearOrden',
...
function guardarServi() {
    ...
    if (check == 0) {
        ...
        for (var i = 0; i < rowsS; i++) {
            if (opeS[i].value == posOrd) {
                $('#TSnoOpe' + i).val(posOrd);
                $('#TSnoS' + i).val(noSer);
                $('#TScant' + i).val(cant);
                $('#TSpreEs1' + i).val(precEs1);
                $('#TSpreEs2' + i).val(precEs2);
                $('#TSgpoA2' + i).val(gpoA2);
                $('#TSclsC' + i).val(clsC);
                $('#TStxtS' + i).val(txtS);
                $('#TSumdS' + i).val(umdS);
                $('#TScont' + i).val(contMat);
            }
        }
...
//FUNCIONES MATCH'S TABALA OPERACIONES
...
function BtnHideTO() {
    ...
    var mDs = $("input[name^='match_TOdesOpe']");
...
// VALIDACIONES CANTIDADES Y LONGITUDES
...
function getExisNumMat(mat) {
    var type;
    var dataSend = "&mater=" + mat;
...
//FUNCIONES CLAVE ORDEN TABLA OPERACIONES
function popUpSta() {
    var sta = $('#TOsta' + posSta).val();
    status = sta;
    posOrd = $('#TOope' + posSta).val();
