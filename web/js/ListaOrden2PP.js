$(document).ready(function () {
    ...
    $('#regresar').click(function () {
        $(location).attr('href', 'ListaOrdenesPP.jsp')
    });
    ...
    $('#btnVis').click(function () {        
        ...
        $.ajax({
            ...
            success: function (data) {               
                if (data == 1) {
                    window.location.href = "VisualizarOrdenes2PP.jsp?ord=" + send + "&peticion=peticionVisualizarOrdenes&tipo=1";
                } else {
                    window.location.href = "VisualizarOrdenes2PP.jsp?ord=" + send + "&peticion=peticionVisualizarCreaOrdenes&tipo=2";
                }
            }
        });
    });
    $('#btnNot').click(function () {
        var send = $('#numordenmsg').html();
        window.location.href = "CrearNotificacionesPP.jsp?orden=" + send;
    });
});
function startTime() {
    today = new Date();
