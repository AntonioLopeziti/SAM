...
$(document).ready(function () {
    ...
    $('#ejecutar').click(function () {
        ...
        if (garticulo.val().length > 0) {
            if (validarDato("grupoarticulo", "ValidarGArticulo", "&garticulo=" + garticulo.val()) == false) {
                msgMatch(3, "images/advertencia.PNG", "audio/saperror.wav", garticulo.val().toUpperCase());
                garticulo.val('');
                garticulo.focus();
                return;
            }
        ...
        if (lote.val().length > 0) {
            if (validarDato("lote", "ValidarLote", "&lote=" + lote.val()) == false) {
                msgMatch(7, "images/advertencia.PNG", "audio/saperror.wav", lote.val().toUpperCase());
                lote.val('');
                lote.focus();
                return;
            }
        ...
        switch (i) {
            ...
            case 1:
                ...
                v.keypress(function (e) {
                    ...
                    if (tecla == 13) {
                        if (v.val().length > 0) {
                            if (validarDato("grupoarticulo", "ValidarGArticulo", "&garticulo=" + v.val()) == true) {
                                borrarmsg();
                            } else {
                                msgMatch(3, "images/advertencia.PNG", "audio/saperror.wav", v.val().toUpperCase());
                                v.val('');
                                v.focus();
                            }
                        }
            ...
            case 4:
                ...
                v.keypress(function (e) {
                    ...
                    if (tecla == 13) {
                        if (v.val().length > 0) {
                            if (validarDato("lote", "ValidarLote", "&lote=" + v.val()) == true) {
                                borrarmsg();
                            } else {
                                msgMatch(7, "images/advertencia.PNG", "audio/saperror.wav", v.val().toUpperCase());
                                v.val('');
                                v.focus();
                            }
                        }
            ...
            case 5:
                v.click(function () {
                    VerPosMa(-1);
                    if ($('#chsum').attr("checked", true)) {
                        $('#chtras').attr("checked", false);
                    }
                });
                break;
            case 6:
                v.click(function () {
                    VerPosMa(-1);
                    if ($('#chtras').attr("checked", true)) {
                        $('#chsum').attr("checked", false);
                    }
                });
                break;
        }
    ...
    $.each(arrm, function (i, v) {
        ...
        switch (i) {
            ...
            case 1:
                v.click(function () {
                    MostrarVentanaModal('VentanaModalGrupoArticulo', 'handle2', 'BusGArtiulos', 'numAcMax2');
                });
                break;
            case 2:
                v.click(function () {
    ...
    function ConsultaGarticulo() {
        var acc = "ConsultaGrupoArticulo";
        $.ajax({
            async: false,
            type: 'GET',
            url: 'peticionInventario',
            contentType: "application/x-www-form-urlencoded",
            processData: true,
            data: "Accion=" + acc + "&v1=" + $('#BusGArtiulos').val() + "&v2=" + $('#BusDenGArt').val() + "&v3=" + $('#numAcMax2').val(),
            success: function (data) {
                if (data == 0) {
                    msgMatch(1, "images/aceptar.png", "audio/sapmsg.wav");
                } else {
                    $('#BuscarParamGrupoArticulo').hide();
                    $('#ConsultaTablaGArticulo').show();
                    $('#CargarDatosGArticulo').html(data);
                    fnc('table-scrollGArticulo', 'fixedYGArticulo');
                    borrarmsg();
                }
            }
        });
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == 0) {
                    ...
                    MostrarVentanaModal('VentanaModalLote', 'handle5', '', '');
                    $('#CargarDatoslote').html(data);
                    fnc('table-scrolllo', 'fixedYlo');
                    borrarmsg();
                }
            }
        });
