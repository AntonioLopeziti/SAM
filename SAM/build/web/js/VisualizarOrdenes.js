...
function enviarDatos(ord) {
    ...
    $.ajax({
        ...
        success: function (rs) {
            if (rs == 1) {
                window.location.href = "VisualizarOrdenes2.jsp?ord=" + ord + "&peticion=" + petVis + "&tipo=" + rs;
            } else if (rs == 2) {
                window.location.href = "VisualizarOrdenes2.jsp?ord=" + ord + "&peticion=" + petCre + "&tipo=" + rs;
            } else {
                //error
                var BE = document.createElement('audio');
                BE.src = "audio/saperror.wav";
