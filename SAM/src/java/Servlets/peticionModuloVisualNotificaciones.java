 ...
 */
@WebServlet(name = "peticionModuloVisualNotificaciones", urlPatterns = {"/peticionModuloVisualNotificaciones"})
public class peticionModuloVisualNotificaciones extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            switch (Accion) {
                ...
                case "ConsultaOrden":
                    ...
                    LinkedList<ordenes_pm_notificaciones> or = ACC_OrdenesPMNotificaciones.ObtenerInstancia().OrdenesMatchNOT(CtdMax, Orden, TxtBrv);
                    if (or.size() > 0) {
                        out.println("<table>");
                        out.println("<tbody>");
                        for (int i = 0; i < or.size(); i++) {
                            ordenes_pm_notificaciones tec = ACC_OrdenesPMNotificaciones.ObtenerInstancia().OrdenesMatchNOText(or.get(i).getNum_orden());
