...
@WebServlet(name = "peticionVisualizarOrden2", urlPatterns = {"/peticionVisualizarOrden2"})
public class peticionVisualizarOrden2 extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            switch (Accion) {
                ...
                case "CargarDatosCabeceraSAP":
                    plan_orden po = ACC_Orden.ObtenerInstancia().CargarCabeceraSAP(orden);
                    planop plop = ACC_Orden.ObtenerInstancia().CargarFirstOpeSAP(orden);
                ...
                case "CargarTablaOpSAP":
                    ArrayList<planop> hr = ACC_Orden.ObtenerInstancia().CargarOperacionesSAP(orden);
                    if (hr.size() > 0) {
                        out.println("<table>");
                        out.println("<tbody>");
                        for (x = 0; x < hr.size(); x++) {
                            ...
                            out.println("<td>" + hr.get(x).getUbitec() + "</td>");
                            out.println("<td>" + hr.get(x).getNum_equipo() + "</td>");
                ...
                case "CargarTablaCompSAP":
                    ArrayList<componentes> com = ACC_Orden.ObtenerInstancia().CargarComponentesSAP(orden);
                    if (com.size() > 0) {
                        out.println("<table>");
                        out.println("<tbody>");
                ...
                case "GetDatosTablaSAP":
                    ...
                    if (clave.equals(pm02)) {
                        ArrayList<servicios_pm> sc = ACC_ServiciosPM.ObtenerInstancia().CargarServiciosSAP(or, opera);
                        if (sc.size() > 0) {
                            out.println("<table>");
                            out.println("<tbody>");
                            for (int i = 0; i < sc.size(); i++) {
                    ...
                    break;
                case "CargarEmplzSAP":
                    oe = ACC_OrdenesEmplazamiento.ObtenerInstancia().CargarDataEmplazamientoSAP(orden);
                    js = new JSONArray();
                ...
                case "CargarPlanifSAP":
                    op = ACC_OrdenesPlanificacion.ObtenerInstancia().CargarDataPlanificacionSAP(orden);
                    js = new JSONArray();
                    ...
                    break;
                case "CargarCtrlSAP":
                    oc = ACC_OrdenesControl.ObtenerInstancia().CargarDataCtrlSAP(orden);
                    js = new JSONArray();
