 ...
 * @author Antonio
 */
@WebServlet(name = "peticionCargarServiciosSolpedMod", urlPatterns = {"/peticionCargarServiciosSolpedMod"})
public class peticionCargarServiciosSolpedMod extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String Solped = request.getParameter("NumSol");
            String posici = request.getParameter("Posicion");
            String tab = request.getParameter("EDIT");
            ...
            switch (Accion) {
                case "CargarServicios":
                    out.println("<table>");
                    out.println("<tbody>");
                    if (posici == null || posici == "" || posici == "0") {
                        for (int i = 0; i < 10; i++) {
                            out.println("<tr>"
                        ...
                        int v;
                        ArrayList<SolpedServicios> s = ACC_Servicios.ObtenerInstancia().CargarServiciosMOD(Solped, posici, u);
                        if (s.size() > 0) {
                            for (v = 0; v < s.size(); v++) {
                                out.println("<tr>"
                                        + "<td><input type=\"checkbox\" class=\"TDSER1\" name=\"ServicesDel\"></td>"
