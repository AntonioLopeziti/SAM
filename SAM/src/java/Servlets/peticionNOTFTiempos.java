...
import AccesoDatos.ACC_Notificaciones_sam;
 ...
 */
@WebServlet(name = "peticionNOTFTiempos", urlPatterns = {"/peticionNOTFTiempos"})
public class peticionNOTFTiempos extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (ACC_Ordenes_pm_notificaciones.ObtenerInstancia().POSNOCRE_INSERTA(folpnc, ord, ope, honot, fenot, durp1, trrep1, nf, nofip1, durp2) == true) {
                if (ACC_Ordenes_pm_notificaciones.ObtenerInstancia().CANOCRE_INSERTA(folicn, ord, ope, honot, fenot, fecco, trrep1)) {
                    if (ACC_Folios.ObtenerIstancia().ActualizarFolioNOT(condicion1, fa)) {
                        out.println(0 + ",NO" + fo.getFolioActual());
                        if ("X".equals(nf)) {
                            if (ACC_Pm03_3_notificaciones.ObtenerInstancia().PONActual("X", ord, ope)) {
                                out.println(0 + ",NO" + fo.getFolioActual());
                            } else {
                                out.println(1);
                            ...
                            if (ACC_Pm03_3_notificaciones.ObtenerInstancia().PONActual("", ord, ope)) {
                                out.println(0 + ",NO" + fo.getFolioActual());
                            } else {
                                out.println(1);
