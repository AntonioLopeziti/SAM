...
public class ACC_HojasRuta {
    ...
    // VISUALIZAR HOJAS DE RUTA PP
    public ArrayList<hojas_de_ruta> ConsultaVisualizarHRPP(String Equipo, String centro, String alter) {
        ArrayList<hojas_de_ruta> hojaR = new ArrayList<>();
        Conexion cnx = new Conexion();
        Connection con = cnx.ObtenerConexion();
        PreparedStatement ps = null;
        ResultSet rs = null;
        String sql = "{CALL PP.HojasRuta_VisHojasRuta(?,?,?)}";
        try {
            ps = con.prepareStatement(sql);
            ps.setString(1, Equipo);
            ps.setString(2, centro);
