<%-- 
    Document   : VistaStockMaterial
    Created on : 24/08/2018, 09:58:05 AM
    Author     : AROQUE
--%>
...
<script>
    ...
    function back() {
        $(location).attr('href', 'ReporteStockMaterial.jsp');
    }
    function loadtab() {
        var mat = '<%=material%>';
        var alm = '<%=almacen%>';
        var cen = '<%=centro%>';
        var acc = "<%=ACC%>";
        var par = "Accion=" + acc + "&material=" + mat + "&almacen=" + alm + "&centro=" + cen;
        $.ajax({
            async: false,
            type: 'GET',
